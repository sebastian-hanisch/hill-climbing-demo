"""Kandidatenlisten + Don't-Look-Bits fuer 2-opt: derselbe Zug wie in hc_algorithm, aber statt nach jedem Zug
...
Delta-Formel und Gueltigkeitspruefung sind wortgleich aus hc_algorithm._delta_2opt/_valid_pairs uebernommen
(a = min(pos[c1], pos[c2]), b = max(...): delta = D[a,b] + D[a+1,b+1] - D[a,a+1] - D[b,b+1], gueltig wenn
b >= a+2 und nicht (a==0 und b==n-1)) - das vermeidet einen Vorzeichenfehler aus eigener Herleitung.
...
def dlb_descend(D, start, cand, seed=0, max_evaluations=None):
    """Ein Kandidatenlisten- + Don't-Look-Bit-2-opt-Abstieg (erste Verbesserung). Eine Bewertung = ein geprueftes
    Kandidatenpaar (c1, c2) - dieselbe Einheit wie 'bewertete Nachbarn' im vollen Abstieg (hc_algorithm.descend)."""
    n = len(start)
